...
function doPost(e) {
  ...
  try {
    ...
    try {
      // Intentar leer campo payload de form POST
...
function saveRelevamiento(data, email) {
  ...
  /* 0. Crear edificio automáticamente si no existe */
  var edificioId = '';
  var edSheet = SS.getSheetByName(HOJAS.EDIFICIOS);
  var nombreEdificio = (data.identificacion && data.identificacion.nombre) ? data.identificacion.nombre : '';
  
  if (nombreEdificio && edSheet) {
    var edData = edSheet.getDataRange().getValues();
    var edHeaders = edData[0];
    var iNombreEd = edHeaders.indexOf('nombre');
    var found = false;
    for (var ei = 1; ei < edData.length; ei++) {
      if (String(edData[ei][iNombreEd]).toLowerCase() === nombreEdificio.toLowerCase()) {
        edificioId = String(edData[ei][0]);
        found = true;
        break;
      }
    }
    if (!found) {
      edificioId = 'ED-' + Date.now();
      appendRow(HOJAS.EDIFICIOS, {
        id_edificio:            edificioId,
        nombre:                 nombreEdificio,
        numero_establecimiento: (data.identificacion && data.identificacion.numero) ? data.identificacion.numero : '',
        tipo:                   (data.identificacion && data.identificacion.tipo) ? data.identificacion.tipo : '',
        nivel:                  '',
        direccion:              (data.identificacion && data.identificacion.direccion) ? data.identificacion.direccion : '',
        delegacion:             (data.identificacion && data.identificacion.delegacion) ? data.identificacion.delegacion : '',
        localidad:              (data.identificacion && data.identificacion.localidad) ? data.identificacion.localidad : '',
        latitud:                (data.identificacion && data.identificacion.latitud) ? data.identificacion.latitud : '',
        longitud:               (data.identificacion && data.identificacion.longitud) ? data.identificacion.longitud : '',
        foto_fachada:           '',
        etapa_id:               (data.gestion && data.gestion.etapa) ? data.gestion.etapa : '',
        zona_id:                (data.gestion && data.gestion.zona) ? data.gestion.zona : '',
        inspector_id:           (data.gestion && data.gestion.inspector) ? data.gestion.inspector : email,
        telefono_fijo:          (data.identificacion && data.identificacion.telefono) ? data.identificacion.telefono : '',
        email_institucional:    (data.identificacion && data.identificacion.email) ? data.identificacion.email : '',
        plano_implantacion:     '',
        plano_edificio:         '',
        activo:                 true,
      });
  ...
  appendRow(HOJAS.RELEVAMIENTOS, {
    id_relevamiento:      id,
    edificio_id:          edificioId || (data.gestion && data.gestion.edificio_id ? data.gestion.edificio_id : ''),
